...
class SwiftComponentHelperSecurityTest < ActionView::TestCase
  ...
  setup do
    ...
    @original_swift_ui_configuration = SwiftUIRails.configuration
    SwiftUIRails.configuration = SwiftUIRails::Configuration.new
  end

  teardown do
    SwiftUIRails.configuration = @original_swift_ui_configuration
  end
  ...
  test "prevents RCE by rejecting arbitrary class names" do
    ...
    dangerous_names.each do |name|
      assert_raises(SwiftUIRails::SecurityError, "Should reject dangerous component name: #{name}") do
    ...
    allowed_names.each do |name|
      ...
      if Object.const_defined?(component_class_name)
        ...
        begin
          ...
          assert_not_instance_of SwiftUIRails::SecurityError, e, "#{name} should be allowed"
        end
      else
  ...
  test "logs security events" do
    ...
    assert_includes log_output, "Attempted to instantiate unauthorized component"
    assert_includes log_output, "Kernel"
  end
