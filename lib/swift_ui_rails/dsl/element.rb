    ...
    class Element
      ...
      def add_stimulus_action(event_type, &block)
        ...
        unless existing_controller&.include?(controller_name)
          @attributes["data-controller"] = existing_controller.present? ? 
            "#{existing_controller} #{controller_name}" : controller_name
        end
        
        # Add the action
        existing_actions = @attributes["data-action"]
        new_action = "#{event_type}->#{controller_name}#handleAction"
        @attributes["data-action"] = existing_actions.present? ? 
          "#{existing_actions} #{new_action}" : new_action
      ...
      def style(style_string)
        ...
        if style_string =~ /javascript:|vbscript:|on\w+\s*=/i
          Rails.logger.warn "[SECURITY] XSS pattern detected in style: #{style_string}"
          return self
        end
        
        # Check data: URLs separately with a simpler pattern
        if style_string.include?('data:') && style_string !~ /data:image\/(?:png|jpg|jpeg|gif|webp|svg\+xml);/i
