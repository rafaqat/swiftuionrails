...
export default class extends Controller {
  ...
  handleReload(data) {
    if (data.type === "reload") {
      console.log("SwiftUI Rails: Reloading due to changes in:", data.files)
      ...
      setTimeout(() => {
        if (this.shouldUseHotReload(data.files)) {
          this.hotReload(data.files)
        } else {
          this.fullReload()
        }
      }, this.delayValue)
  ...
  shouldUseHotReload(files) {
    // Use hot reload for component and story files
    return files.every(file => 
      file.includes("component") || file.includes("stories")
    )
  }
