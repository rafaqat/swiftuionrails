...
export default class extends Controller {
  ...
  initializeMonaco() {
    ...
    if (window.monaco) {
      this.createEditor(container, initialCode)
    } else {
      // Wait for loader
  ...
  createEditor(container, initialCode) {
  ...
  setupCompletionProvider() {
    monaco.languages.registerCompletionItemProvider('ruby', {
      provideCompletionItems: async (model, position) => {
        ...
        const response = await fetch('/v2/playground/completions', {
          method: 'POST',
          headers: {
  ...
  filterComponents(event) {
    constquery = event.target.value.toLowerCase()
  ...
  loadFavorites() {
    ...
    favorites.forEach(fav => {
      ...
      div.innerHTML = `
        <button class="text-sm text-gray-700 hover:text-blue-600 text-left truncate flex-1" data-action="click->playground-v2#loadExample" data-playground-v2-code-param="${this.escapeHtml(fav.code)}">
  ...
  shareCode() {
    ...
    const url = `${window.location.origin}/v2/playground?code=${encoded}`
