...
export default class extends Controller {
  ...
  submit(event) {
    ...
    if (!this.isValidSearchTerm(value)) {
      console.debug('Search submission blocked: validation failed')
      this.shake()
      return
    }
    
    if (value.length < this.minLength) {
      console.debug('Search submission blocked: minimum length requirement')
      this.shake()
      return
    }
    ...
    // Security: Log search event without exposing user data
    console.debug('Search form submitted successfully')
