...
module Playground
  ...
  class FastLexer
    ...
    def self.receiver_for_method_completion(src)
      ...
      # Find the immediate receiver (the last method call before the dot)
      # We need to work backwards from the dot to find the complete method call
      if pre_dot =~ /(\w+)\s*\([^)]*\)\s*$/
        # Simple case: method("arg")
        [Regexp.last_match(1)]
      elsif pre_dot =~ /(\w+)\s*$/
        # Method without parentheses
        [Regexp.last_match(1)]
      else
        # Fallback to chain_before_dot for complex cases
