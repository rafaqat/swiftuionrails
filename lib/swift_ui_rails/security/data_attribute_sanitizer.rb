    ...
    module DataAttributeSanitizer
      ...
      class << self
        ...
        def safe_data_attributes(options = {})
          ...
          # Stimulus action
          if options[:action]
            attrs["data-action"] = sanitize_stimulus_action(options[:action])
          end
          
          # Stimulus controller
          if options[:controller]
            attrs["data-controller"] = sanitize_stimulus_controller(options[:controller])
          end
          
          # Stimulus target
          if options[:target]
            attrs["data-#{options[:controller]}-target"] = sanitize_stimulus_target(options[:target])
          end
        ...
        def sanitize_stimulus_action(action)
          ...
          unless controller_method.match?(/\A[a-zA-Z][a-zA-Z0-9\-_]*#[a-zA-Z][a-zA-Z0-9_]*\z/)
            return ""
          end
