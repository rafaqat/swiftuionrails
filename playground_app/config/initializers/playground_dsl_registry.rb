...
Rails.application.reloader.to_prepare do
  ...
  registry.register(:hstack, {
    ...
    examples: [ 
      "hstack { }",
      "hstack(spacing: 8) { }",
      'hstack(justify: :between) { text("Left"); text("Right") }',
      'hstack(justify: :center) { text("Centered") }',
      'hstack(justify: :around) { text("A"); text("B"); text("C") }'
    ]
  })
  ...
  registry.register(:form, {
    ...
    modifiers: %w[padding margin background border],
  ...
  # Table components
  registry.register(:simple_table, {
    description: "Simple table with headers and rows",
    parameters: { 
      headers: { type: "Array", required: true, default: '["Name", "Role", "Status"]' },
      rows: { type: "Array", required: true, default: '[["John", "Admin", "Active"], ["Jane", "User", "Inactive"]]' }
    },
    modifiers: %w[border rounded shadow padding margin],
    examples: [ 
      'simple_table(headers: ["Name", "Role"], rows: [["John", "Admin"], ["Jane", "User"]])',
      'simple_table(headers: ["Product", "Price"], rows: [["iPhone", "$999"], ["MacBook", "$1299"]])'
    ]
  })

  registry.register(:data_table, {
    description: "Advanced data table with sorting, search, and rich formatting",
    parameters: { 
      data: { type: "Array", required: true, default: '[{name: "John", role: "Admin"}]' },
      columns: { type: "Array", required: true, default: '[{key: :name, label: "Name"}]' },
      title: { type: "String", required: false, default: "Data Table" },
      sortable: { type: "Boolean", required: false, default: true },
      paginate: { type: "Boolean", required: false, default: false },
      elevation: { type: "Integer", required: false, default: 2, options: [0, 1, 2, 3, 4, 5] }
    },
    modifiers: %w[margin padding],
    examples: [ 
      'data_table(data: users, columns: [{key: :name, label: "Name"}])',
      'data_table(data: products, columns: [{key: :name, label: "Product"}, {key: :price, label: "Price", format: :currency}], title: "Products")',
      'data_table(data: employees, columns: [{key: :name, label: "Name", format: :avatar_with_text}, {key: :status, label: "Status", format: :badge}], search: {placeholder: "Search..."}, sortable: true)'
    ]
  })
  ...
  registry.register(:table, {
    ...
    parameters: {},
    modifiers: %w[width border rounded],
    examples: [ "table { }" ]
  })

  registry.register(:thead, {
    description: "Table header",
    parameters: {},
    modifiers: %w[background border],
    examples: [ "thead { }" ]
  })

  registry.register(:tbody, {
    description: "Table body",
    parameters: {},
    modifiers: %w[background],
    examples: [ "tbody { }" ]
  })
  ...
  registry.register(:tr, {
    ...
    parameters: {},
    modifiers: %w[hover background border],
    examples: [ "tr { }" ]
  })
  ...
  registry.register(:td, {
    ...
    parameters: {},
    modifiers: %w[padding text_align],
    examples: [ "td { }" ]
  })
  ...
  registry.register(:th, {
    ...
    parameters: {},
    modifiers: %w[padding text_align font_weight],
    examples: [ "th { }" ]
  })
