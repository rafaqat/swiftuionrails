...
class ComponentGeneratorSecurityTest < Rails::Generators::TestCase
  ...
  destination Rails.root.join("tmp/generators/component_generator_security")
  ...
  test "prevents injection through prop names" do
    ...
    dangerous_props.each do |prop|
      assert_raises Thor::Error do
        generator = generator_for("SafeComponent", prop)
        generator.send(:validate_props!)
        generator.send(:parsed_props)
      end
    end
  end
  ...
  test "rejects Ruby reserved words as prop names" do
    ...
    reserved_words.each do |word|
      assert_raises Thor::Error do
        generator_for("ValidComponent", "#{word}:String").send(:parsed_props)
      end
    end
  end
  ...
  def prepare_isolated_destination
    @isolated_destination_root = Dir.mktmpdir("component-generator-", Rails.root.join("tmp").to_s)
  ...
  def generator_for(name, *props)
    SwiftUIRails::Generators::ComponentGenerator.new([name, *props])
  end
end
